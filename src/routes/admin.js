/***************************
        ADMIN ROUTES
*****************************/

import express from "express";
import suppliersRoutes from "../domains/admin/suppliers/routes.js";
import beansRoutes from "../domains/admin/beanology/routes.js";
import roastersRoutes from "../domains/admin/roasters/routes.js";
import roastsRoutes from "../domains/admin/roastology/routes.js";

const adminRouter = express.Router();

adminRouter.use('/suppliers', suppliersRoutes);
adminRouter.use('/beanology', beansRoutes);
adminRouter.use('/roasters', roastersRoutes);
adminRouter.use('/roastology', roastsRoutes);

export default adminRouter